  ...
  <Members>
    ...
    <Member MemberName="ListTablesWithHttpMessagesAsync">
      ...
      <Docs>
        ...
        <param name="basic">
            基本的なスイッチでは、テーブルを一覧表示するときに返される情報のレベルを示します。 基本的な場合は true、のみ database_name、schema_name、table_name と各、それ以外の場合のテーブルのすべてのメタデータが返されるテーブルのバージョンが返されます。 既定では false です。
            省略可能。
            </param>
        <param name="customHeaders">
            追加されるヘッダーを要求します。
