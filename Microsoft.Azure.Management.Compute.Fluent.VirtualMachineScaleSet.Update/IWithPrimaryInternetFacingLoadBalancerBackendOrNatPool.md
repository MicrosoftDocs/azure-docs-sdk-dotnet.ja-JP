<Type Name="IWithPrimaryInternetFacingLoadBalancerBackendOrNatPool" FullName="Microsoft.Azure.Management.Compute.Fluent.VirtualMachineScaleSet.Update.IWithPrimaryInternetFacingLoadBalancerBackendOrNatPool">
  ...
  <Docs>
    <summary>
            仮想マシンのスケールのステージの設定に、バックエンドを関連付けるには更新を許可するプールや、選択したインターネットからの着信 NAT プールのロード バランサー仮想マシン スケール セット内のプライマリ ネットワーク インターフェイスを使用します。
            </summary>
  ...
  <Members>
    <Member MemberName="WithPrimaryInternetFacingLoadBalancerBackends">
      ...
      <Docs>
        <param name="backendNames">バックエンドの名前。</param>
        <summary>
            関連付けられた、指定されたインターネットに接続されたは、仮想マシン スケール セット内のプライマリ ネットワーク インターフェイスを持つバランサー バックエンドを読み込みます。
            </summary>
