<Type Name="IWithPrimaryInternalLoadBalancerNatPool" FullName="Microsoft.Azure.Management.Compute.Fluent.VirtualMachineScaleSet.Update.IWithPrimaryInternalLoadBalancerNatPool">
  ...
  <Docs>
    <summary>
            仮想マシンのスケールのステージに、仮想マシン スケール セット内のプライマリ ネットワーク インターフェイスで選択されている内部ロード バランサーの着信 NAT プールを関連付けるには更新を許可するを設定します。
            </summary>
  ...
  <Members>
    <Member MemberName="WithPrimaryInternalLoadBalancerInboundNatPools">
      ...
      <Docs>
        <param name="natPoolNames">選択されているロード バランサーに既存の着信 NAT プールの名前。</param>
        <summary>
            関連付けます指定された内部ロード バランサーの着信 NAT プールを仮想マシン スケール セット内のプライマリ ネットワーク インターフェイス。
            </summary>
