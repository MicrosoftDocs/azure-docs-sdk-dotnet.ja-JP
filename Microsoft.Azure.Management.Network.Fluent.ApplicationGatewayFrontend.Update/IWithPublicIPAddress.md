<Type Name="IWithPublicIPAddress" FullName="Microsoft.Azure.Management.Network.Fluent.ApplicationGatewayFrontend.Update.IWithPublicIPAddress">
  ...
  <Docs>
    <summary>
            <span data-ttu-id="c64f9-101">アプリケーション ゲートウェイをインターネットに接続するとで使用できるようにするには既存のパブリック IP アドレスを指定できるようにアプリケーション ゲートウェイ フロント エンドの定義の段階です。</span><span class="sxs-lookup"><span data-stu-id="c64f9-101">The stage of an application gateway frontend definition allowing to specify an existing public IP address to make the application gateway available at as Internet-facing.</span></span>
            </summary>
