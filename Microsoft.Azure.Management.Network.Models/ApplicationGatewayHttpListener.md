<Type Name="ApplicationGatewayHttpListener" FullName="Microsoft.Azure.Management.Network.Models.ApplicationGatewayHttpListener">
  ...
  <Members>
    <Member MemberName=".ctor">
      ...
      <Docs>
        <summary>
            ApplicationGatewayHttpListener クラスの新しいインスタンスを初期化します。
            </summary>
        <remarks>To be added.</remarks>
      </Docs>
    </Member>
    <Member MemberName=".ctor">
      ...
      <Docs>
        ...
        <param name="frontendIPConfiguration">アプリケーション ゲートウェイのフロント エンド IP 構成のリソースです。</param>
        <param name="frontendPort">アプリケーション ゲートウェイのフロント エンド ポート リソースです。</param>
        <param name="protocol">プロトコルです。 使用可能な値が含まれます 'Http'、'Https'。</param>
        <param name="hostName">HTTP リスナーのホスト名です。</param>
        <param name="sslCertificate">Application gateway の SSL 証明書リソースです。</param>
        <param name="requireServerNameIndication">プロトコルは https の場合にのみ適用されます。 SNI をマルチホストを有効にします。</param>
        <param name="provisioningState">HTTP リスナー リソースのプロビジョニング状態。 使用可能な値が: '更新'、'削除' および '失敗' です。</param>
