  ...
  <Members>
    ...
    <Member MemberName=".ctor">
      ...
      <Docs>
        ...
        <param name="addressPrefix">サブネットのアドレス プレフィックス。</param>
        <param name="networkSecurityGroup">NetworkSecurityGroup リソースの参照です。</param>
        <param name="routeTable">RouteTable リソースの参照です。</param>
        <param name="serviceEndpoints">サービス エンドポイントの配列。</param>
        <param name="ipConfigurations">サブネットを使用して、ネットワーク インターフェイス IP 構成への参照の配列を取得します。</param>
        <param name="resourceNavigationLinks">サブネットを使用して外部リソースへの参照の配列を取得します。</param>
