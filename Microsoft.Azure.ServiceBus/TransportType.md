<Type Name="TransportType" FullName="Microsoft.Azure.ServiceBus.TransportType">
  ...
  <Members>
    <Member MemberName="Amqp">
      ...
      <Docs>
        <summary>
            Over TCP、AMQP を使用します。
            <remarks>これは、既定値です。これは、ポート 5671 で実行されます。</remarks></summary>
      </Docs>
    </Member>
    <Member MemberName="AmqpWebSockets">
      ...
      <Docs>
        <summary>
            AMQP を使用して Websocket 経由で
            </summary>
        <remarks>これは、wss URI スキームとポート 443 で実行されます。 ポート 5671 へのトラフィックがブロックされているシナリオで使用します。 プロキシ接続をセットアップするには、システムの既定のプロキシを構成してください。 現在プロキシは、net451 + フレームワークでのみサポートされます。</remarks>
      </Docs>
    </Member>
  </Members>
</Type>