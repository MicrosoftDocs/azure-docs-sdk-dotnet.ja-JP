<Type Name="RetryExponential" FullName="Microsoft.ServiceBus.RetryExponential">
  ...
  <Interfaces />
  <Docs>
    <summary>再試行ポリシーの実装を表します。 メッセージング操作を再試行する必要が毎回の再試行の間の遅延はずらして、指数的に大きくなります。</summary>
    <remarks>再試行ポリシーでは、次の側面を許可: <list type="bullet"><item>ポリシーが常に優先、<seealso cref="P:Microsoft.ServiceBus.Messaging.ClientEntity.OperationTimeout" />ため、次の再試行間隔が操作に時間を超える場合、その再試行が終了されます</item>。<item>ときに、ポリシーがだけ再試行<seealso cref="P:Microsoft.ServiceBus.Messaging.MessagingException.IsTransient" />は true</item> 。<item>ポリシーが遅延する必要があります、追加の 10 秒例外は、する場合によって、さらに、<seealso cref="T:Microsoft.ServiceBus.Messaging.ServerBusyException" /></item></list></remarks>
  </Docs>
  <Members>
    <Member MemberName=".ctor">
      ...
      <Docs>
        <param name="minBackoff">最小値は、待機する値が無効バックアップします。 この値はより大きいか等しいを作成できません<see cref="P:Microsoft.ServiceBus.RetryExponential.MaximumBackoff" />、それ以外の場合の操作できない可能性がありますを再試行します。</param>
        <param name="maxBackoff">元に戻し値まで待機する最大です。 この値はより大きいか等しいを作成できません、<see cref="P:Microsoft.ServiceBus.Messaging.MessagingFactorySettings.OperationTimeout" />値、それ以外の場合の操作できないことがありますを再試行します。</param>
        <param name="maxRetryCount">再試行ポリシー インスタンスは、再試行の時間間隔を計算する最大回数。 再試行の回数は、この値を超えている場合、再試行が終了によって残りの操作時間がある場合でもです。</param>
